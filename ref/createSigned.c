...
main()
{
    ...
    sprintf(fn_req, "keypairs_ref_%s-simple.rsp", EXPORTINFO); //should be changed from _ref_ to _java_ or _csharp_

    sprintf(fn_rsp, "signed_ref_%s-simple.rsp",EXPORTINFO); //should be _bcmode_ref_
    ...
    do {
        ...
        if ( (ret_val = crypto_sign(sm, &smlen, m, mlen, sk,fp_rsp)) != 0) {
